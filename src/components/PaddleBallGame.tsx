...
const PaddleBallGame = ({ onBackToSelection, onTransitionToWheelOfFortune }: PaddleBallGameProps) => {
    ...
    const updateBall = (canvas: HTMLCanvasElement) => {
        ...
        // Use a flag to prevent multiple score increments per goal
        ...
        if (ball.x + ball.radius < 0) {
            ...
            console.log("updateBall() ---> setScore");
            setScores(prev => {
    ...
    const gameLoop = () => {
        ...
        console.log("gameLoop() ---> updateBall()");
        updateBall(canvas);
    ...
    const handleMouseMove = (e: MouseEvent) => {
        ...
        if (x < canvas.width / 2) {
            gameObjectsRef.current.leftPaddle.y = Math.max(0, Math.min(canvas.height - PADDLE_HEIGHT, y - PADDLE_HEIGHT / 2));
        }
        // Control right paddle with mouse on right half
        else {
            gameObjectsRef.current.rightPaddle.y = Math.max(0, Math.min(canvas.height - PADDLE_HEIGHT, y - PADDLE_HEIGHT / 2));
        }
    };
    ...
    const startGame = () => {
        ...
        console.log("handleMouseLeave ----> Why???");
    ...
    return (
        <div className="h-full w-full flex flex-col bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 relative overflow-hidden">
            ...
            <div className="flex-1 p-4 relative">
                <canvas ref={canvasRef} className="w-full h-full border-2 border-slate-600/50 rounded-2xl bg-slate-900/50 shadow-2xl touch-none" />
                ...
                {showWinnerMessage && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm pointer-events-none z-20">
                        <div className="text-center">
                            <div className="text-6xl md:text-8xl font-bold text-white mb-4 animate-pulse">🏆</div>
                            <div className="text-4xl md:text-6xl font-bold text-white mb-6">{winnerText}</div>
                            <div className="text-2xl md:text-3xl text-slate-300 mb-4">Congratulations!</div>
                            <div className="text-lg md:text-xl text-slate-400">Transitioning to next phase...</div>
                        </div>
                    </div>
                )}
